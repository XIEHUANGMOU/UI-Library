local Library = (function()
	...
	local LoadingRoot = nil
	local LoadingRingConn = nil
	local LoadingSpin = 0
	local localEXEName = "XHM Ultra"
	...
	local function ShowLoading()
		...
		New("UIGradient", {
			...
			Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, Color3.fromRGB(9, 9, 11)),
				ColorSequenceKeypoint.new(0.55, Color3.fromRGB(22, 22, 30)),
		...
		for i = 1, 8 do
			...
			local cx = 60 + math.cos(ang) * 52 - 4
			local cy = 60 + math.sin(ang) * 52 - 4
			local Dot = New("Frame", {
				Name = "Dot" .. i,
		...
		local prompt = "正在加载执行器..."
		local fullText = "执行器：" .. localEXEName
		local printedPrompt = 0
		task.spawn(function()
			while printedPrompt < #prompt do
				printedPrompt = printedPrompt + 1
				SubText.Text = string.sub(prompt, 1, printedPrompt)
				task.wait(0.08)
			end
			SubText.Text = prompt
			task.wait(0.15)
			local nameLabel = New("TextLabel", {
				...
				Position = UDim2.new(0.5, 0, 0.5, 158),
				AnchorPoint = Vector2.new(0.5, 0.5),
				BackgroundTransparency = 1,
				Font = Enum.Font.GothamSemibold,
				Text = "",
		...
		LoadingRingConn = RunService.RenderStepped:Connect(function()
			LoadingSpin = LoadingSpin + 4
			Ring.Rotation = LoadingSpin
			LogoImg.Rotation = -LoadingSpin * 0.7
			local breath = 0.55 + 0.45 * (0.5 + 0.5 * math.sin(os.clock() * 3))
			LoadingRoot.BackgroundTransparency = 0
			TitleText.TextTransparency = breath * 0.35
		end)
	end
	local function FinishLoading()
		...
		if LoadingRingConn then
			LoadingRingConn:Disconnect()
			LoadingRingConn = nil
		end
		if LoadingRoot then
			TweenService:Create(LoadingRoot, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				BackgroundTransparency = 1,
			}):Play()
		end
